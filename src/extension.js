...
const PAGE_SIZE = 100;
const CONTROLLER_TAGS = { smolpaws: "insider", insiderrole: "controller" };
...
const isController = (c) =>
  c?.tags?.smolpaws === "insider" && c?.tags?.insiderrole === "controller";
...
function messageText(event) {
  ...
  if (message && ["user", "assistant"].includes(message.role)) {
    const text = (message.content || [])
      ...
      .map((item) => item.text)
      .join("\n");
    if (text.trim()) return { role: message.role, text };
  }
  if (
...
export function activate(host) {
  ...
  const unregister = host.registerPage("projects", ({ container, navigate }) =>
    ...
    <p data-role="notice" class="cat-notice" role="status" aria-live="polite"></p><h3 data-copy="recent"></h3><div data-role="history" class="cat-history"></div><p data-copy="partialHistory" class="cat-muted"></p><p data-copy="voice" class="cat-voice"></p></aside></div>`;
      ...
      const notice = (text) => {
        if (alive) role("notice").textContent = text;
      };
      ...
      const open = (id) => navigate(`/conversations/${encodeURIComponent(id)}`);
      ...
      function updateBoardFilters() {
        ...
        action("project").replaceChildren(
          option("", t("all")),
          ...[...new Set(board.map(workspaceOf).filter(Boolean))].map((path) =>
            option(path, path),
          ),
        );
      ...
      async function discoverControllers(firstPage, generation) {
        ...
        while (next) {
          ...
          if (!Array.isArray(page.items)) throw new Error(t("discoveryFailed"));
      ...
      async function refreshController() {
        ...
        try {
          ...
          if (!history.childElementCount) history.textContent = t("noMessages");
          renderControls();
        } catch (error) {
          if (alive && generation === readGeneration)
            notice(t("catError", { message: errorText(error) }));
        } finally {
          if (alive && generation === readGeneration)
            timer = setTimeout(
              refreshController,
          ...
          if (!state.controller) {
            ...
            const info = await request("/api/conversations", "POST", {
              ...
              agent_launch_additions: {
                system_message_suffix_append: `<INSIDER_CONTROLLER>\nThe runtime identifies this conversation as the active Insider Cat controller. Backend: ${host.backend.id}. Controller ID: ${id}.\nThe Apps page supplies selected-worker context as data with each request. This conversation has only its configured profile tools; no browser client tools, voice connection, shared SmolPaws memory, or scheduler are added by this App. Only claim operations supported by your actual tools.\n\n${INSIDER_INSTRUCTIONS}\n</INSIDER_CONTROLLER>`,
              },
            ...
            state.controller = info;
            submitted = true;
            await request(`${detailPath(state.controller.id)}/events`, "POST", {
              role: "user",
              content,
              run: true,
            });
      ...
      action("load-more").addEventListener("click", () => void loadBoard(true));
      ...
      action("controller").addEventListener("change", () => {
        ...
        state.controller =
          controllers.find((c) => c.id === action("controller").value) || null;
        state.newRequested = false;
        state.uncertain = false;
        state.controllerInvalid = false;
        state.mutationNotice = null;
      ...
      action("new-cat").addEventListener("click", () => {
        if (state.busy || state.uncertain) return;
